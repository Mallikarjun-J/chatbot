    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 flex flex-col h-full max-h-[calc(100vh-120px)] lg:max-h-[calc(100vh-140px)]">
            <div className="p-4 border-b dark:border-gray-700">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">Powered by Gemini with Live Knowledge Base</p>
            </div>
                    ...
                    >
                        {msg.role === 'model' && (
                         ...
                         <div className={`max-w-xs md:max-w-md p-3 rounded-2xl ${msg.role === 'user' ? 'bg-indigo-500 text-white rounded-br-none' : 'bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-bl-none'}`}>
                            ...
                            {msg.metadata && msg.role === 'model' && (
                                <div className="mt-3 pt-3 border-t border-gray-300 dark:border-gray-600">
                                    <h4 className="text-xs font-semibold text-gray-600 dark:text-gray-400 mb-2">
                                        🧠 Knowledge Base Used:
                                    </h4>
                                    <div className="text-xs text-gray-600 dark:text-gray-400 space-y-1">
                                        {msg.metadata.announcementsUsed! > 0 && (
                                            <div className="flex items-center gap-1">
                                                <span className="font-medium">📢</span> {msg.metadata.announcementsUsed} announcements
                                            </div>
                                        )}
                                        {msg.metadata.liveDataFetched && (
                                            <div className="flex items-center gap-1">
                                                <span className="font-medium">🌐</span> Live data from college website
                                            </div>
                                        )}
                                        {msg.metadata.placementsFound! > 0 && (
                                            <div className="flex items-center gap-1">
                                                <span className="font-medium">💼</span> {msg.metadata.placementsFound} placement records
                                            </div>
                                        )}
                                        {msg.metadata.eventsFound! > 0 && (
                                            <div className="flex items-center gap-1">
                                                <span className="font-medium">📅</span> {msg.metadata.eventsFound} events
                                            </div>
                                        )}
                                        {msg.metadata.model && (
                                            <div className="flex items-center gap-1 text-gray-500 dark:text-gray-500">
                                                <span className="font-medium">🤖</span> {msg.metadata.model}
                                            </div>
                                        )}
                                    </div>
                                </div>
                            ...
                            {/* Sources */}
                             {msg.sources && msg.sources.length > 0 && (
                                <div className="mt-3 pt-3 border-t border-gray-300 dark:border-gray-600">
                                    <h4 className="text-xs font-semibold flex items-center gap-1.5 text-gray-600 dark:text-gray-400">
                                                ...
                                                {source.type === 'live_scraping' && (
                                                    <div className="bg-green-50 dark:bg-green-900/20 p-2 rounded">
                                                        ...
                                                        <div className="text-gray-600 dark:text-gray-400 mt-1">
                                                            {source.count} items • Scraped: {source.scrapedAt ? new Date(source.scrapedAt).toLocaleTimeString() : 'just now'}
                                                        </div>
                                                    </div>
                                                )}
                                                {source.type === 'announcements' && (
                                                    <div className="bg-blue-50 dark:bg-blue-900/20 p-2 rounded">
                                                        <div className="font-semibold text-blue-700 dark:text-blue-400">
                                                            📢 Announcements Database
                                                        </div>
                                                        <div className="text-gray-600 dark:text-gray-400">
                                                            {source.count} announcements{source.latest ? ` • Latest: "${source.latest}"` : ''}
                                                        </div>
                                                    </div>
                                                )}
                                                {source.type === 'placements' && (
                                                    <div className="bg-purple-50 dark:bg-purple-900/20 p-2 rounded">
                                                        <div className="font-semibold text-purple-700 dark:text-purple-400">
                                                            💼 Placement Records
                                                        </div>
                                                        <div className="text-gray-600 dark:text-gray-400">
                                                            {source.count} placement announcements found
                                                        </div>
                                                    </div>
                                                )}
                                                {source.type === 'events' && (
