...
const App: React.FC = () => {
  ...
  // Generate notifications when new announcements are detected
   useEffect(() => {
    // On first load, if user just logged in and there are announcements, create notifications for recent ones
    if (prevAnnouncementsRef.current.length === 0) {
      prevAnnouncementsRef.current = announcements;
      
      // Create notifications for the 3 most recent announcements when user first logs in
      if (user && announcements.length > 0) {
        const recentAnnouncements = announcements.slice(0, 3);
        const initialNotifications: Notification[] = recentAnnouncements.map(ann => ({
          id: `notif-${ann.id}`,
          message: `📢 ${ann.title}`,
          timestamp: ann.date,
          isRead: false,
